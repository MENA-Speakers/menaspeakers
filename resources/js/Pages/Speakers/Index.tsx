      ...
      <section className="py-12">
        <div className="max-w-7xl mx-auto  sm:px-6   px-6 lg:px-0">
          ...
          <form onSubmit={formik.handleSubmit}>
                ...
                <div className={'flex space-x-6 items-center'}>
                  <div className="flex-grow relative">
                    <Input name="query" placeholder={'Search'}
                           className={'w-full border rounded-3xl py-3 px-8'} value={formik.values.query}
            ...
            <div className={`flex items-center  gap-6 ${query ? 'justify-between' : 'justify-end'}`}>
              {
                query && (
                  <p>
                    {
                      speakers.data.length
                    } results found for <span className="font-semibold">"{query}"</span>
                  </p>
                )
              }
              ...
              <Button type={'button'} onClick={() => setShowFilters(!showFilters)}
                      className={
                        `rounded-2xl border border-mena-brand group
              ${showFilters ?
                ...
                Filter by
                <SlidersHorizontal
                  className={`h-6 w-6 ml-2 stroke-1 ${showFilters ? 'group-hover:text-white' : 'group-hover:text-mena-brand'}`}/>
              </Button>
            </div>

            {
              showFilters && (
                <div className={'grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-20 pt-6'}>
                  <div>
                    <Label htmlFor="location"
                           className="block text-sm sr-only font-medium text-gray-700">Categories</Label>
                    <Select
